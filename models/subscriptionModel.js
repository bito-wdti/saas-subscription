...
export const SubscriptionModel = {
  ...
  async create(subscription) {
    try {
      const { stripe_customer_id, product_id, customer_name, customer_email, payment_method } = subscription;
      ...
      if (!product_id) {
        const error = new Error('product_id é obrigatório');
        error.code = 'VALIDATION_ERROR';
        error.statusCode = 400;
        throw error;
      }
      ...
      const stripeSubscription = await createSubscription(stripe_customer_id, product_id);
